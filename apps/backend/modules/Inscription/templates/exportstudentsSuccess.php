...
$student =& $workbook->addformat();
$student->set_align('right');
$student->set_color('black');
$student->set_size(10);
$student->set_bold();

// CABECERA
$worksheet->write(0, $col, "Nom d'infant", $header);
$col = 1;
...
foreach ($inscripcions as $key => $inscription) {
    $insc = InscriptionPeer::retrieveByPK($inscription['id']);

    if ($isNewName($inscription)) {
        $studentName = $inscription['student_name']
            . ' ' . $inscription['student_primer_apellido']
            . ' ' . $inscription['student_segundo_apellido'];

        $worksheet->write($row, 0, utf8_decode($studentName), $student);
        $row++;
    }
    ...
    if ($key > $limitRowsExcel) {
        ...
        foreach ($columns as $key => $value) {
            if (in_array($key, $selectedColumns)) {
                $worksheet->write(0, $col, utf8_decode($value), $center);
                $col++;
            }
        }
