...
abstract class BaseGrupoPeer {
	...
	const CLASS_DEFAULT = 'lib.model.inscriptions.Grupo';
	...
	const CENTRO_ID = 'grupo.CENTRO_ID';
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_COLNAME => array (GrupoPeer::ID, GrupoPeer::NOMBRE, GrupoPeer::CENTRO_ID, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_COLNAME => array (GrupoPeer::ID => 0, GrupoPeer::NOMBRE => 1, GrupoPeer::CENTRO_ID => 2, ),
	...
	{
		include_once 'lib/model/inscriptions/map/GrupoMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.inscriptions.map.GrupoMapBuilder');
	}
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(GrupoPeer::CENTRO_ID);
    ...
    foreach (sfMixer::getCallables('BaseGrupoPeer:addDoSelectRS:addDoSelectRS') as $callable)
    {
      call_user_func($callable, 'BaseGrupoPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseGrupoPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseGrupoPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseGrupoPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseGrupoPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseGrupoPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseGrupoPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseGrupoPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseGrupoPeer', $values, $con, $ret);
    }
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/inscriptions/map/GrupoMapBuilder.php';
	Propel::registerMapBuilder('lib.model.inscriptions.map.GrupoMapBuilder');
}
