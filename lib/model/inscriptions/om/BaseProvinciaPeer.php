...
abstract class BaseProvinciaPeer {
	...
	const CLASS_DEFAULT = 'lib.model.inscriptions.Provincia';
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_COLNAME => array (ProvinciaPeer::ID, ProvinciaPeer::NOMBRE, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_COLNAME => array (ProvinciaPeer::ID => 0, ProvinciaPeer::NOMBRE => 1, ),
	...
	{
		include_once 'lib/model/inscriptions/map/ProvinciaMapBuilder.php';
		return BasePeer::getMapBuilder('lib.model.inscriptions.map.ProvinciaMapBuilder');
	}
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(ProvinciaPeer::NOMBRE);
    ...
    foreach (sfMixer::getCallables('BaseProvinciaPeer:addDoSelectRS:addDoSelectRS') as $callable)
    {
      call_user_func($callable, 'BaseProvinciaPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseProvinciaPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseProvinciaPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseProvinciaPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseProvinciaPeer', $values, $con, $ret);
    }
...
if (Propel::isInit()) {
		...
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
			...
			require_once 'lib/model/inscriptions/map/ProvinciaMapBuilder.php';
	Propel::registerMapBuilder('lib.model.inscriptions.map.ProvinciaMapBuilder');
}
