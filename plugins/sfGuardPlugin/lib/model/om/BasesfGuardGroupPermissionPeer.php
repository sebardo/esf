...
abstract class BasesfGuardGroupPermissionPeer {
	...
	{
		include_once 'plugins/sfGuardPlugin/lib/model/map/sfGuardGroupPermissionMapBuilder.php';
		return BasePeer::getMapBuilder('plugins.sfGuardPlugin.lib.model.map.sfGuardGroupPermissionMapBuilder');
	}
	
	public static function getPhpNameMap()
	{
		if (self::$phpNameMap === null) {
			$map = sfGuardGroupPermissionPeer::getTableMap();
	...
	const COUNT = 'COUNT(sf_guard_group_permission.GROUP_ID)';
	const COUNT_DISTINCT = 'COUNT(DISTINCT sf_guard_group_permission.GROUP_ID)';
	...
	public static function doCount(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		if ($distinct || in_array(Criteria::DISTINCT, $criteria->getSelectModifiers())) {
			$criteria->addSelectColumn(sfGuardGroupPermissionPeer::COUNT_DISTINCT);
		} else {
			$criteria->addSelectColumn(sfGuardGroupPermissionPeer::COUNT);
		}
		...
		$rs = sfGuardGroupPermissionPeer::doSelectRS($criteria, $con);
		if ($rs->next()) {
			return $rs->getInt(1);
		} else {
						return 0;
	...
	{
		return sfGuardGroupPermissionPeer::populateObjects(sfGuardGroupPermissionPeer::doSelectRS($criteria, $con));
	}
	
	public static function doSelectRS(Criteria $criteria, $con = null)
	{

    foreach (sfMixer::getCallables('BasesfGuardGroupPermissionPeer:addDoSelectRS:addDoSelectRS') as $callable)
    {
	...
	public static function doSelectJoinsfGuardGroup(Criteria $c, $con = null)
	{
		...
		$startcol = (sfGuardGroupPermissionPeer::NUM_COLUMNS - sfGuardGroupPermissionPeer::NUM_LAZY_LOAD_COLUMNS) + 1;
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getsfGuardGroup(); 				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
										$temp_obj2->addsfGuardGroupPermission($obj1); 					break;
				}
			}
			if ($newObject) {
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				...
				$temp_obj3 = $temp_obj1->getsfGuardPermission(); 				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addsfGuardGroupPermission($obj1); 					break;
				}
			}

			if ($newObject) {
				$obj3->initsfGuardGroupPermissions();
	...
	public static function doDeleteAll($con = null)
	{
	 ...
	 public static function doDelete($values, $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			$criteria = clone $values; 		} elseif ($values instanceof sfGuardGroupPermission) {
			...
			$criteria->add(sfGuardGroupPermissionPeer::GROUP_ID, $vals[0], Criteria::IN);
			$criteria->add(sfGuardGroupPermissionPeer::PERMISSION_ID, $vals[1], Criteria::IN);
		}
...
if (Propel::isInit()) {
			try {
		BasesfGuardGroupPermissionPeer::getMapBuilder();
	} catch (Exception $e) {
		Propel::log('Could not initialize Peer: ' . $e->getMessage(), Propel::LOG_ERR);
	}
} else {
			require_once 'plugins/sfGuardPlugin/lib/model/map/sfGuardGroupPermissionMapBuilder.php';
	Propel::registerMapBuilder('plugins.sfGuardPlugin.lib.model.map.sfGuardGroupPermissionMapBuilder');
}
