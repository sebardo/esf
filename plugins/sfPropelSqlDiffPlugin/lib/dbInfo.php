...
class dbInfo {
  ...
  function getDiffWith(dbInfo $db_info2) {
    ...
    foreach($db_info2->tables as $tablename=>$tabledata) {
      $diff_sql .= $this->getDiffWithCreateOrAlter($db_info2,$tablename);
    }
    ...
    foreach($this->tables as $tablename=>$tabledata) {
      ...
      foreach($tabledata['fields'] as $field=>$fielddata) {
        $mycode = strtolower($fielddata['code']);
        $otherdata = @$db_info2->tables[$tablename]['fields'][$field];
        $othercode = strtolower(@$otherdata['code']);
        if($mycode and !$othercode) {
  ...
  public function executeSql($sql, $connection = null) {
    if(!$connection){
  ...
  function getDiffWithCreateOrAlter(dbInfo $db_info2,$tablename) {
    $diff_sql = '';
    $tabledata = $db_info2->tables[$tablename];
    if(isset($db_info2->tables[$tablename]["completed"])){
      return '';
    }
    if(isset($tabledata["fkeys"])){
      foreach ($tabledata["fkeys"] as $fkey){
        if(isset($db_info2->tables[$fkey["ref_table"]])){
          if($fkey["ref_table"] != $tablename
             && !isset($db_info2->tables[$fkey["ref_table"]]["completed"])){
            $diff_sql .= $this->getDiffWithCreateOrAlter($db_info2,$fkey["ref_table"]);
          }
        }
      }
    }
    ...
    if(!isset($this->tables[$tablename])) {
      ...
      $db_info2->tables[$tablename]["completed"] = true;
      return $diff_sql;
    }
    ...
    if($tabledata['fkeys'] && $this->tableSupportsFkeys($tabledata['type'])) {
      foreach($tabledata['fkeys'] as $fkeyname=>$data) {
        $mycode = $data['code'];
        // rename unless $fkeyname exists
        while(isset($this->tables[$tablename]['fkeys'][$fkeyname])){
          if(preg_match("/(.*_)(\d+)$/",$fkeyname,$r)){
            $newfkname = $r[1].$r[2]+1;
          }else{
            $newfkname .= "_1";
          }
          $mycode = str_replace($fkeyname,$newfkname,$mycode);
          $fkeyname = $newfkname;
        }
