...
class TCPDFFichaListado extends TCPDF
{
	const SANGRADO = 2;
	const FONT = "din";
	const FONT_SIZE = 10;
    ...
    {
        $headers = array(sfContext::getInstance()->getI18N()->__('Alumne / Setmana'),
            sfContext::getInstance()->getI18N()->__('Al·lèrgies'),
            sfContext::getInstance()->getI18N()->__('Grup'),
            sfContext::getInstance()->getI18N()->__('Serveis'),
            sfContext::getInstance()->getI18N()->__('Telèfons'));

        //error_log($this->getPageWidth());

        // With: 297.000083333
        $this->headersWidth = array(70, 70, 45, 65, 37);

        $this->SetXY(5, 17);
        $this->SetFont(self::FONT, 'n', 12);

        for ($i = 0; $i < count($headers); $i++)
        {
            $this->Cell($this->headersWidth[$i], 8, $headers[$i], array('LTRB' => array('dash' => 0, 'width' => 0)), 0, 'C', 0, '', 0, false, 'M', 'T');
        }
    ...
    {
    	$this->SetFont(self::FONT, self::FONT_STYLE, self::FONT_SIZE);
        $this->Cell(0, 0, $this->getPage() , 0, false, 'R', 0, '', 0, false, 'M', 'M');
    }
